...
def train_SDGCCA(hyper_dict):
    ...
    for cv in tqdm(range(5), desc='CV...'):
        ...
        if not os.path.exists('resultAHmini'):
            os.makedirs('resultAHmini')
            ...
            for lr in hyper_dict['lr']:
                for reg in hyper_dict['reg']:
                    for lcor in hyper_dict['lcor']:
                        ...
                        model = SDGCCA_4_M(m1_embedding_list, m2_embedding_list, m3_embedding_list, m4_embedding_list, top_k).to(
                            hyper_dict['device'])
        ...
        # 将DataFrame写入CSV文件
        output_df.to_csv(f"resultAHmini/AHminicv{cv}.csv", index=False)
        # Find best K
        ...
        torch.save(model.state_dict(), f"resultAHmini/ADGCCAmini_AH{cv}.pt")
